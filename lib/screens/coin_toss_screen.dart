...
class _CoinTossScreenState extends State<CoinTossScreen> with SingleTickerProviderStateMixin {
  ...
  String? _callingTeam;
  String _selectedCall = 'HEADS';
  String? _coinResult;
  String? _tossWinner;
  String _selectedDecision = 'Bat First';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final match = Provider.of<MatchController>(context, listen: false).currentMatch;
      if (match != null) {
        setState(() {
  ...
  void _onProceedToConfirmation() {
    final controller = Provider.of<MatchController>(context, listen: false);
    MatchModel? match = controller.currentMatch;
    if (match == null || _callingTeam == null || _coinResult == null || _tossWinner == null) {
      return;
    }
    ...
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => const MatchConfirmationScreen()),
    );
  }
                    ...
                    child: Column(
                      children: [
                        ...
                        Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              const Text('VS', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900, color: AppTheme.coinGold)),
                              Text(
                                match.teamB,
                        ...
                        if (controller.isFlipping) ...[
                          const Text(
                            'Flipping Coin...',
                            style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: AppTheme.coinGold, letterSpacing: 1.2),
                          ),
                          const SizedBox(height: 10),
                          const SizedBox(
                            width: 26,
                            height: 26,
                            child: CircularProgressIndicator(strokeWidth: 3, color: AppTheme.coinGold),
                          ),
                        ...
                        if (_coinResult != null && !controller.isFlipping && _fadeAnim != null && _scaleAnim != null) ...[
                              ...
                              child: Container(
                                ...
                                child: Column(
                                  children: [
                                    const Text(
                                      '🪙 COIN RESULT',
                                      style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: AppTheme.textMuted, letterSpacing: 1.5),
                                    ),
                                    const SizedBox(height: 6),
                                    Text(
                                      _coinResult!,
                                      style: const TextStyle(fontSize: 32, fontWeight: FontWeight.w900, color: AppTheme.coinGold),
                                    ),
                                    const Divider(color: Color(0xFF2E5749), height: 24),
                                    Row(
                                      mainAxisAlignment: MainAxisAlignment.center,
                                      children: [
                                        const Icon(Icons.emoji_events, color: AppTheme.primaryEmerald, size: 28),
                          ...
                          TextButton.icon(
                            ...
                            label: const Text('Re-flip Coin', style: TextStyle(color: AppTheme.textMuted)),
                          ),
                        ],
                      ],
                    ),
  ...
  Widget _buildCallingTeamCard({required String teamName, required Color color}) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? color.withValues(alpha: 0.15) : AppTheme.cardBgLight,
        ...
        child: Center(
          child: Text(
            teamName,
            style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isSelected ? color : Colors.white),
          ),
        ),
      ),
    );
      ...
      child: AnimatedContainer(
        ...
        child: Column(
          children: [
            ...
            Text(
              option,
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: isSelected ? Colors.black : Colors.white),
            ),
          ],
        ),
      ),
  ...
  Widget _buildDecisionCard(String decision, String emoji) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? AppTheme.primaryEmerald.withValues(alpha: 0.15) : AppTheme.cardBg,
          borderRadius: BorderRadius.circular(14),
          border: Border.all(
            color: isSelected ? AppTheme.primaryEmerald : const Color(0xFF2E5749),
            width: isSelected ? 2 : 1,
          ),
        ),
        child: Column(
          children: [
            ...
            Text(
              decision,
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isSelected ? AppTheme.primaryEmerald : Colors.white),
            ),
          ],
        ),
      ),
